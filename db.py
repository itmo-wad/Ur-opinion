...
def getcountteam(teamid):
    return teammembers.count({"teamid": teamid})
...
def next_editor(taskid):
    ...
    tasks.update_one({'_id': ObjectId(taskid)},
                     {"$set": {"currenteditor": currenteditor}})

    # check if currenteditor = count of team member.. it means that the task
    # finished
