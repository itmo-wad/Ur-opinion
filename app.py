    ...
    # otherwise
        ...
        if request.method == 'POST':
            ...
            if log_r(username, password):
                return redirect("/")
        # if get method , or wrong logging in
...
def internal_error(e):
    requests.post("https://notify.bot.codex.so/u/1L9N0D7I",
...
handler = logging.handlers.RotatingFileHandler(
    'logs/app.log', maxBytes=32, backupCount=2)
handler.setLevel(logging.DEBUG)
handler.setFormatter(logging.Formatter(
    '%(asctime)s [in %(pathname)s:%(lineno)d]: %(message)s'))
